...
class Tank(AttackUnit):
    # 시즈모드 : 탱크를 지상에 고정시켜, 더 높은 파워로 멀리 공격 가능. 이동불가.
    seize_delvloped = False # 시즈모드 개발여부
    def __init__(self):
        AttackUnit.__init__(self, "탱크", 150, 1, 35)
        self.seize_mode = False #시즈 모드(해제상태)
    def set_seize_mode(self):
        if Tank.seize_delvloped == False:
            return
        # 현재 시즈모드가 아닐 때  -> 시즈모드
...
class Wraith(FlyableAttackUnit):
    # 클로킹 : 상대방이 보이지 않도록 운둔처리 됨.
    clocked_delvloped = False # 클로킹모드 개발여부
    def __init__(self):
        FlyableAttackUnit.__init__(self, "레이스", 80, 20, 5)
        self.clocking_mode = False #클로킹 모드(해제상태)
    def set_clocking_mode(self):
        if Wraith.clocked_delvloped == False:
            return
        # 현재 클로킹 모드가 아닐 때  -> 클로킹 모드
        if self.clocking_mode == False:
            print("{0} : 클로킹 모드로 전환합니다.".format(self.name))
            self.clocking_mode = True
        # 현재 클로킹 모드 일때 -> 클로킹 모드 해제
        else:
...
Tank.seize_delvloped = True
print("[알림] 탱크 시즈 모드 개발이 완료 되었습니다.")
Wraith.clocked_delvloped = True
print("[알림] 레이스 클로킹 모드 개발이 완료 되었습니다.")
